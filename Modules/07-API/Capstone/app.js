...
function loadCat(data) {
    ...
    data.forEach(cat => {
        ...
        const elImg = document.createElement('img');
        const catImageURL = `https://cdn2.thecatapi.com/images/${cat.reference_image_id}.jpg`;
        elImg.src = catImageURL;
        elImg.onerror = () => {
            elImg.onerror = null; // Disable further error handling
            elImg.src = './assets/missingcat.png'; // Fallback image source
        };
        ...
        elButton.classList.add('card__button')
         ...
         // set button directly to wikepedia once click the button
         elButton.type = 'button';
         elButton.textContent = 'Wikipedia URL';
         elButton.addEventListener('click', () => {
             window.open(cat.wikipedia_url);
         });
 

        // Set button attributes and action onclick.(zacqueleong method)
